        ...
        if ($this->num_pages > 10) {
            ...
            for ($i = 1; $i <= $this->num_pages; $i++) {
                if ($this->range[0] > 2 and $i == $this->range[0]) {
                    $this->return .= " <button type=\"button\" class=\"btn btn-icon rounded-pill bg-white\">...</button> ";
                }

                if ($i == 1 or $i == $this->num_pages or in_array($i, $this->range)) {
                    $this->return .= ($i == $this->current_page and $_GET['Page'] != 'All') ? "<a title=\"Go to page $i of $this->num_pages\" class=\"btn btn-icon rounded-pill btn-primary\" href=\"javascript:pages_change($i);\">$i</a> " : "<a class=\"btn btn-icon rounded-pill btn-light\" title=\"Go to page $i of $this->num_pages\" href=\"" . 'javascript:pages_change(' . $i . ');' . "\">$i</a> ";
                }
                if ($this->range[$this->mid_range - 1] < $this->num_pages - 1 and $i == $this->range[$this->mid_range - 1]) {
                    $this->return .= " <button type=\"button\" class=\"btn btn-icon rounded-pill bg-white\">...</button> ";
                }
            }
            $this->return .= (($this->current_page != $this->num_pages and $this->items_total >= 10) and ($_GET['Page'] != 'All')) ? "<a class=\"btn btn-link btn-icon\" href=\"" . 'javascript:pages_change(' . $next_page . ');' . "\"><svg xmlns=\"http://www.w3.org/2000/svg\" width=\"24\" height=\"24\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"green\" stroke-width=\"2\" stroke-linecap=\"round\" stroke-linejoin=\"round\" class=\"feather feather-chevron-right\"><polyline points=\"9 18 15 12 9 6\"></polyline></svg></a>\n" : "<span class=\"btn btn-link btn-icon\" href=\"#\"><svg xmlns=\"http://www.w3.org/2000/svg\" width=\"24\" height=\"24\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"grey\" stroke-width=\"2\" stroke-linecap=\"round\" stroke-linejoin=\"round\" class=\"feather feather-chevron-right\"><polyline points=\"9 18 15 12 9 6\"></polyline></svg></span>\n";
        } else {
            for ($i = 1; $i <= $this->num_pages; $i++) {
                $this->return .= ($i == $this->current_page) ? "<a class=\"btn btn-icon rounded-pill btn-primary\" href=\"javascript:pages_change($i);\">$i</a> " : "<a class=\"btn btn-icon rounded-pill btn-light\" href=\"" . 'javascript:pages_change(' . $i . ');' . "\">$i</a> ";
            }
        }
