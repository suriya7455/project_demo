...
function mission(){
  $(document).on('click','#mission',function(e){
    ...
    if (name == undefined) {
      Swal.fire('กรุณาเลือกนักผจญภัยก่อน');
      return false;

    }
    var st = $(this).attr('data-st');
    if (st == '0' || st == '') {
      Swal.fire('ไม่สามารถรับมอบภารกิจได้ เนื่องจากนักผจญภัยท่านไม่ได้เลือกรับภารกิจ');
      return false;
    }

    Swal.fire({
      title: 'ต้องการรับมอบภารกิจจากนักผจญภัย "'+name+'" นี้หรือไม่',
      showDenyButton: true,
      ...
      if (result.isConfirmed) {
        ...
        var idn = $(this).attr('data-idmen');
        var name =  $('#name').val();
        var links = lines+'prosessquestin.php?m=missionRequest';
        $.post(links,{'idq':idq,'idn':idn,'name':name},function(data){
         var obj = JSON.parse(data);
         if (obj.success == '1') {

          Swal.fire({
            icon: 'success',
            title: 'รับมอบภารกิจเรียบร้อย',
            text: ''
          });
 ...
 function clicklistdatamenber(){
  $(document).on('click','#lismenbber',function(e){
    ...
    $('#mission').attr('data-idq',idq);
    $('#mission').attr('data-idmen',id);
    $('#mission').attr('data-st',st);

    $('#cancelagreemenber').attr('data-idq',idq);
    $('#cancelagreemenber').attr('data-idmen',id);
    $('#cancelagreemenber').attr('data-st',st);
...
function cancelagreemenber(){
  $(document).on('click','#cancelagreemenber',function(e){
    e.stopImmediatePropagation();
    var name =  $('#name').val();
    ...
    var st = $(this).attr('data-st');
    if (st == '0' || st == '') {
      Swal.fire('ไม่สามารถยกเลิกนักผจญภัยได้ เนื่องจากนักผจญภัยท่านไม่ได้เลือกรับภารกิจ');
      return false;
    }

    Swal.fire({
      title: 'ต้องการยกเลิกนักผจญภัย "'+name+'" ที่รับภารกิจนี้หรือไม่',
      showDenyButton: true,
      confirmButtonText: 'ตกลง',
    }).then((result) => {
      /* Read more about isConfirmed, isDenied below */
      if (result.isConfirmed) {
        ...
        var idn = $(this).attr('data-idmen');
        var name =  $('#name').val();
        var links = lines+'prosessquestin.php?m=cancenRequest';
        $.post(links,{'idq':idq,'idn':idn,'name':name},function(data){
         var obj = JSON.parse(data);
         if (obj.success == '1') {
          ...
          Swal.fire({
            icon: 'success',
            title: 'ยกเลิกนักผจญภัยที่รับภารกิจเรียบร้อย',
            text: ''
          });
          $('.numlistadd').text(obj.num);
          setTimeout(function(){ location.reload(); }, 1000);
  ...
  $(document).on('click','#agreemenber',function(e){
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        var idn = $(this).attr('data-idmen');
        var name =  $('#name').val();
        var links = lines+'prosessquestin.php?m=putRequest';
        $.post(links,{'idq':idq,'idn':idn,'name':name},function(data){
         var obj = JSON.parse(data);
         if (obj.success == '1') {
          ...
          Swal.fire({
            icon: 'success',
            title: 'ตกลงรับนักผจญภัยเรียบร้อย',
            text: ''
          });
          $('.numlistadd').text(obj.num);
          setTimeout(function(){ location.reload(); }, 1000);
        ...
        if (obj.success == '2'){
           Swal.fire({
            icon: 'error',
            title: 'รับนักผจญภัยเต็มภารกิจแล้ว',
            text: ''
          });
